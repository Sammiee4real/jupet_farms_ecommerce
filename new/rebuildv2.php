	  ...
	  function the_actual_rebuild_algorithm(){
			...
			while($row = mysqli_fetch_array($qry_cron_forms)){
					...
					if($countRecord == 0){
						...
						$qry1="select `frid` from formresponse_staging where fid='$form_id111' ORDER BY latlong";
						$res1=mysqli_query($dbc,$qry1);
						if(mysqli_num_rows($res1)>0){
						$query_del = "DELETE FROM `formresponse_staging` WHERE `fid`='$form_id111' and `status`=0";
